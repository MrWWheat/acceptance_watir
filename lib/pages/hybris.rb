...
class Pages::Hybris < Pages::Base
  ...
  def ensure_first_time_browser_boot!
    ...
    unless ( username.present? || login_link.present? )
      ...
      @browser.goto url
      @browser.wait_until { username.present? || login_link.present? }
  ...
  def is_logged_out?
    @browser.wait_until { (login_link.present? && !logout_link.present?) || 
                          (!login_link.present? && logout_link.present?) }
    login_link.present? 
  end

  def login!(user)
    @login_page ||= Pages::Hybris::Login.new(@config)
  ...
  def logout!
    ...
    logout_link.click  #From Pages::Hybris::Layout
    @browser.wait_until { !logout_link.present? }
